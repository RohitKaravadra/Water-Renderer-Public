...
public:
	static float fresnelDielectric(float cosTheta, float eta)
	{
		...
		// Safe cost
		float cosThetaT = std::sqrtf(std::max(0.0f, 1.0f - sin2ThetaT));

		float denomParl = (cosTheta + eta * cosThetaT);
		float denomPerp = (eta * cosTheta + cosThetaT);

		if (denomParl == 0.0f || denomPerp == 0.0f)
			return 1.0f; // treat as full reflection to avoid NaN

		float fParl = (cosTheta - eta * cosThetaT) / denomParl;
		float fPerp = (eta * cosTheta - cosThetaT) / denomPerp;

		return 0.5f * (fParl * fParl + fPerp * fPerp);
	}
...
class PlanktonBSDF : public BSDF
{
	float g = 0.9f;
	float eta = 1.05f; // Plankton refractive index
	float invEta = 1.f / eta;
public:
	PlanktonBSDF() = default;
	PlanktonBSDF(Texture* _albedo)
	{
		albedo = _albedo;
	}
	Vec3 sample(const ShadingData& shadingData, Sampler* sampler, Color& reflectedColour, float& pdf)
...
public:
	...
	Vec3 sample(const ShadingData& shadingData, Sampler* sampler, Color& reflectedColour, float& pdf)
	{
		...
		float cosThetaO = std::max(fabsf(wi.z), 1e-6f);
		reflectedColour = reflectedColour * pdf / cosThetaO;
