...
public:
	...
	PlaneMesh(Vec3 center, Vec3 size, Vec3 part, unsigned int matIndex = 0)
	{
		...
		for (unsigned int i = 0; i < xPart; i++)
		{
			for (unsigned int j = 0; j < yPart; j++)
			{
				...
				t1.init(v00, v01, v10, matIndex);
				t2.init(v10, v01, v11, matIndex);
	...
	void invertNormals()
	{
		...
		{
			t.vertices[0].normal = -t.vertices[0].normal;
			t.vertices[1].normal = -t.vertices[1].normal;
			t.vertices[2].normal = -t.vertices[2].normal;
		}
	}
};
...
public:
	...
	// create mesh
	CubeMesh(Vec3 center, Vec3 size, Vec3 partition, unsigned int matIndex = 0)
	{
		CreateMesh(center, size, partition, matIndex, "\0", "\0", 0.0f);
	}

	// create mesh with normal map
	CubeMesh(Vec3 center, Vec3 size, Vec3 partition,
		std::string normalMap, unsigned int matIndex = 0)
	{
		CreateMesh(center, size, partition, matIndex, normalMap, "\0", 0.0f);
	}

	// create mesh with height map
	CubeMesh(Vec3 center, Vec3 size, Vec3 partition, std::string heightMap,
		float strength, unsigned int matIndex = 0)
	{
		CreateMesh(center, size, partition, matIndex, "", heightMap, strength);
	}

	// create mesh with normal and height map
	CubeMesh(Vec3 center, Vec3 size, Vec3 partition, std::string normalMap,
		std::string heightMap, float strength, unsigned int matIndex = 0)
	{
		CreateMesh(center, size, partition, matIndex, normalMap, heightMap, strength);
	}
		...
		std::string normalMap, std::string heightMap, float strength)
	{
		...
		bool hasNormalMap = !normalMap.empty(), hasHeightMap = !heightMap.empty() && strength > 0;
		...
		Vec3 p[8] = {
			center + Vec3(-half.x, -half.y, -(-half.z)), // 0 - Left-Bottom-Back
			center + Vec3(half.x, -half.y, -(-half.z)),  // 1 - Right-Bottom-Back
			center + Vec3(half.x, half.y, -(-half.z)),   // 2 - Right-Top-Back
			center + Vec3(-half.x, half.y, -(-half.z)),  // 3 - Left-Top-Back
			center + Vec3(-half.x, -half.y, -(half.z)),  // 4 - Left-Bottom-Front
			center + Vec3(half.x, -half.y, -(half.z)),   // 5 - Right-Bottom-Front
			center + Vec3(half.x, half.y, -(half.z)),    // 6 - Right-Top-Front
			center + Vec3(-half.x, half.y, -(half.z))    // 7 - Left-Top-Front
		};
		...
		for (int f = 0; f < 6; ++f)
		{
			if (f == 4) // Top face (+Y) - Partitioned
			{
				int xParts = max(1, (int)partition.x);
				int zParts = max(1, (int)partition.z);

				Vec3 v0 = p[faces[f][0]]; // Left-Back
				Vec3 v1 = p[faces[f][1]]; // Right-Back
				Vec3 v2 = p[faces[f][2]]; // Right-Front
				Vec3 v3 = p[faces[f][3]]; // Left-Front

				Frame frame;
				frame.fromVector(Vec3(0, 1, 0));
					...
					for (int z = 0; z < zParts; ++z)
					{
						...
						if (hasNormalMap)
						{
							for (int i = 0; i < 4; ++i)
							{
								...
								Vec3 normal = sampled * 2.0f - Vec3(1.0f, 1.0f, 1.0f);
								verts[i].normal = frame.toWorld(normal.normalize());
							}
						}

						// Sample Height Map (if available)
						if (hasHeightMap)
						{
							...
							for (int i = 0; i < 4; ++i)
							{
								...
								if (!isEdge)
								{
									float height = heightTex->sample(verts[i].u, verts[i].v).r; // Assuming red channel
									height -= height * 0.5f;									// range from -0.5 to 0.5
									verts[i].p = verts[i].p + verts[i].normal * height * strength;
								}
							}
						}

						// Build 2 Triangles (Reverse Winding Order)
						Triangle t1, t2;
			...
			else // Other faces (single quad, 2 triangles each)
			{
				Vertex v0 = { p[faces[f][0]], normals[f], 0.0f, 0.0f };
				Vertex v1 = { p[faces[f][1]], normals[f], 1.0f, 0.0f };
				Vertex v2 = { p[faces[f][2]], normals[f], 1.0f, 1.0f };
		...
		// Clean up textures if they were loaded
		delete normalTex;
		delete heightTex;
	}

	void removeFaces(const std::string& facesStr)
	{
		...
		while (iss >> face)
		{
			...
			if (it != faceMap.end())
				indicesToRemove.insert(it->second);
		}

		// Rebuild triangle list without unwanted faces
		std::vector<Triangle> filtered;
		for (int i = 0; i < triangles.size(); i += 2)
		{
			int faceIndex = i / 2;
			if (indicesToRemove.count(faceIndex) == 0)
			{
				filtered.push_back(triangles[i]);
				if (i + 1 < triangles.size())
					filtered.push_back(triangles[i + 1]);
			}
		}
