...
#define SQ(x) (x * x)
inline float safeMax(float a, float b = 1e-6f) { return a > b ? a : b; }
...
public:
	...
	Color normalize()
	{
		...
		return maxValue > 0.0f ? Color(r / maxValue, g / maxValue, b / maxValue) : Color(0.0f, 0.0f, 0.0f);
	}
...
public:
	...
	{
		float iv = 1.0f / v;
		return Vec3(x * iv, y * iv, z * iv, w * iv);
	}
	Vec3 operator*(const Vec3 v) const
	{
		return Vec3(x * v.x, y * v.y, z * v.z);
	...
	{
		float iw = 1.0f / w;
		return Vec3(x * iw, y * iw, z * iw, 1.0f * iw);
	}
...
public:
	union
	{
		float a[4][4];
		float m[16];
		struct { Vec3 col1, col2, col3, col4; };
	};
	Matrix() { identity(); }
	...
	Matrix(const Vec3& c1, const Vec3& c2, const Vec3& c3)
	{
		col1 = c1;
		col2 = c2;
		col3 = c3;
		col4 = Vec3(0, 0, 0); // Default w = 0
	}
	void identity()
	{
...
static const Color COLOR_GREY(0.5f, 0.5f, 0.5f);
static const Color COLOR_DARK_GREY(0.25f, 0.25f, 0.25f);
static const Color COLOR_LIGHT_GREY(0.75f, 0.75f, 0.75f);

// Define some common vectors
static const Vec3 V3_ZERO(0.0f, 0.0f, 0.0f);
static const Vec3 V3_ONE(1.0f, 1.0f, 1.0f);
static const Vec3 V3_RIGHT(1.0f, 0.0f, 0.0f);
static const Vec3 V3_UP(0.0f, 1.0f, 0.0f);
static const Vec3 V3_FORWARD(0.0f, 0.0f, -1.0f);
static const Vec3 V3_LEFT(-1.0f, 0.0f, 0.0f);
static const Vec3 V3_DOWN(0.0f, -1.0f, 0.0f);
static const Vec3 V3_BACK(0.0f, 0.0f, 1.0f);