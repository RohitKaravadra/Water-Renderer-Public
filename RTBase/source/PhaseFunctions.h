...
public:
	...
	float sample(const Vec3& wo, Vec3& wi, Sampler* sampler) const override
	{
		...
		Vec3 localDir = SamplingDistributions::uniformSampleSphere(r1, r2);

		// create a frame from outgoing direction 'wo'
		Frame frame;
		frame.fromVector(wo);
	...
	{
		std::cout << "Isotropic Phase Function" << std::endl;
	}

	IsotropicPhase* clone() const override
	{
...
class vMFPhase : public PhaseFunction
{
	...
	Vec3 sampleDirection(const vMFData& data, float r1, float r2) const
	{
		float cosTheta;
		if (data.kappa < 1e-5f)
		{
		...
		{
			float logTerm = std::log(1.0f - r1 + r1 * std::expf(-2.0f * data.kappa));
			cosTheta = 1.0f + (1.0f / data.kappa) * logTerm;
		}
		float sinTheta = std::sqrt(fmax(0.0f, 1.0f - cosTheta * cosTheta));
		float phi = 2.0f * float(M_PI) * r2;

		Vec3 sample(sinTheta * std::cosf(phi), sinTheta * std::sinf(phi), cosTheta);

		Frame frame;
		frame.fromVector(data.mu);
		return frame.toWorld(sample);
	}
public:
	...
	vMFPhase(const std::vector<vMFData>& samples)
	{
		...
		for (auto& data : this->samples)
		{
			...
			float denom = fmax(1e-6f, 4.0f * float(M_PI) * std::sinhf(data.kappa));
			data.norm = data.kappa / denom;
		}
	...
	float sample(const Vec3& wo, Vec3& wi, Sampler* sampler) const override
	{
		...
		// 1. Select a vMF component based on weights
		size_t index = 0;
		if (cumPDF.size() > 1)
		{
		...
		Vec3 localWi = sampleDirection(data, r2, r3);

		Frame woFrame;
		woFrame.fromVector(wo);
		wi = woFrame.toWorld(localWi);
	...
	float evaluate(const Vec3& wo, const Vec3& wi) const override
	{
		...
		Frame frame;
		frame.fromVector(wo);
		Vec3 localWi = frame.toLocal(wi);

		return calculatePdf(localWi);
	}

	void print() const override {
		std::cout << "vMFDistribution with " << samples.size() << " vmfs \n";
